...
import { AdvancedPanel, AdvancedSettings, defaultAdvancedSettings, simpleToAdvanced } from './components/AdvancedPanel'
...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    void window.api.listPresets().then(setPresets).catch(() => setPresets([]))
  }, [])
  ...
  const handleSavePreset = async (name: string, overwriteId?: string): Promise<void> => {
    ...
    const mirrored = simpleToAdvanced({
      selectedTool, codec, compressionMethod, crf, bitrate,
      resizeWidth, resizeHeight, outputFormat,
      audioFormat, audioMode, audioQuality, audioBitrate, wavBitDepth,
      startTime, endTime
    })
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid #222' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <h2 style={{ margin: 0, fontSize: '0.85rem', fontWeight: 600, color: '#999', letterSpacing: '0.07em', textTransform: 'uppercase' }}>Queue</h2>
            {queue.length > 0 && (
              <span style={{ fontSize: '0.72rem', backgroundColor: '#2a2a2a', color: '#777', padding: '1px 7px', borderRadius: '10px', fontWeight: 500 }}>
                {queue.length}
              </span>
            )}
          </div>
        ...
        {queue.length > 0 && (
          ...
          >
            <span>{queue.filter(i => i.status === 'idle' || i.status === 'error').length} pending</span>
            <span className="toggle-all" style={{ cursor: 'pointer', color: '#3b82f6', fontWeight: 500 }} onClick={selectAllIdle}>
              Toggle All
            </span>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {queue.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '40px', color: '#3a3a3a', textAlign: 'center', padding: '0 16px' }}>
              ...
              <p style={{ margin: 0, fontSize: '0.8rem', lineHeight: 1.5 }}>Drop files here<br/>or click Import</p>
            </div>
          )}
          ...
          {queue.map((item, index) => (
            ...
            >
              ...
              {item.status === 'idle' || item.status === 'error' ? (
                ...
                <span style={{ marginRight: '9px', width: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {item.status === 'done' ? (
                    <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#10b981', display: 'inline-block' }} />
                  ) : (
                    <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#f59e0b', display: 'inline-block', animation: 'pulse 1.2s ease-in-out infinite' }} />
                  )}
                </span>
              )}
              ...
              {item.customOutputPath && item.status !== 'done' && (
                <span
                  ...
                  style={{ marginRight: '4px', color: '#3b82f6', display: 'flex', alignItems: 'center', flexShrink: 0 }}
                >
      ...
      <div
        ...
        onMouseLeave={(e) =>
          (e.currentTarget.style.backgroundColor = isDragging === 'left' ? '#3b82f6' : 'transparent')
        }
      />
        ...
        {activeItem ? (
          <>
            ...
            >
              ...
              {activeItem.status === 'done' ? (
                ...
                >
                  ...
                  <h2 style={{ color: '#e8e8e8', margin: '0 0 6px 0', fontSize: '1.1rem', fontWeight: 600 }}>Done</h2>
                  <p style={{ color: '#555', fontSize: '0.78rem', margin: '0 auto 22px auto', maxWidth: '380px', wordBreak: 'break-all', lineHeight: 1.6 }}>
                ...
                <div style={{ textAlign: 'center', padding: '40px', maxWidth: '420px' }}>
                  <div style={{ marginBottom: '14px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)' }}>
                  ...
                  <h2 style={{ color: '#f87171', margin: '0 0 8px 0', fontSize: '1rem', fontWeight: 600 }}>Processing Failed</h2>
                  <p style={{ color: '#666', fontSize: '0.82rem', margin: 0, lineHeight: 1.6 }}>{activeItem.errorMessage}</p>
                </div>
              ) : (
                <MediaPreview
            ...
            <div style={{ marginTop: '16px', textAlign: 'center' }}>
              <h3 style={{ margin: '0 0 4px 0', fontSize: '0.875rem', fontWeight: 500, color: '#ccc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{activeItem.file.name}</h3>
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 500, color: '#404040' }}>Drop files here</p>
            <p style={{ margin: 0, fontSize: '0.78rem', color: '#333' }}>or select from the queue</p>
          </div>
        )}
      </div>
      ...
      <div
        ...
        onMouseLeave={(e) =>
          (e.currentTarget.style.backgroundColor = isDragging === 'right' ? '#3b82f6' : 'transparent')
        }
      />
      ...
      >
        <div style={{ padding: '14px 14px 12px', borderBottom: '1px solid #222' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
            <h2 style={{ margin: 0, fontSize: '0.85rem', fontWeight: 600, color: '#999', letterSpacing: '0.07em', textTransform: 'uppercase' }}>Settings</h2>
          </div>
          {/* Mode switcher */}
          <div style={{ display: 'flex', backgroundColor: '#1a1a1a', borderRadius: '7px', padding: '3px', border: '1px solid #252525' }}>
            <button
            ...
            >
              Advanced
              {advancedMode && (
                <span style={{ fontSize: '0.6rem', backgroundColor: '#3b82f6', color: '#fff', padding: '1px 5px', borderRadius: '8px', fontWeight: 700, letterSpacing: '0.04em' }}>BETA</span>
              )}
            </button>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              <option value="extract_audio" disabled={activeItem?.file.type.startsWith('image/')}>
          ...
          >
            {isAnalyzing ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100px', gap: '10px' }}>
                <div style={{ width: '22px', height: '22px', border: '2px solid rgba(255,255,255,0.06)', borderTop: '2px solid #3b82f6', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {!codec.startsWith('prores') && codec !== 'hap' && (
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...styles.miniButton,
                          backgroundColor: compressionMethod === 'bitrate' ? '#2563eb' : '#1e1e1e',
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '5px' }}>
                    <button className="mini-btn" onClick={() => handleScale(0.5)} style={styles.miniButton}>½</button>
                    <button className="mini-btn" onClick={() => handleScale(0.333)} style={styles.miniButton}>⅓</button>
                    <button className="mini-btn" onClick={() => handleScale(0.25)} style={styles.miniButton}>¼</button>
                    <button className="mini-btn" onClick={() => { if (originalWidth && originalHeight) { setResizeWidth(originalWidth.toString()); setResizeHeight(originalHeight.toString()) } }} style={styles.miniButton}>Reset</button>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <p style={{ margin: 0, fontSize: '0.8rem', color: '#aaa' }}>
                  Drag the handles on the preview timeline to set the GIF in/out points, or press
        ...
        <div style={{ flexShrink: 0, padding: '14px 16px 16px', borderTop: '1px solid #222', backgroundColor: '#171717' }}>
            <label
              style={{
                display: 'flex',
                alignItems: 'center',
                ...
                padding: '9px 10px',
                borderRadius: '8px',
                backgroundColor: replaceOriginal && !forcesExtensionChange ? 'rgba(239,68,68,0.06)' : 'transparent',
            ...
            >
              ...
              <span
                style={{
                  fontSize: '0.82rem',
                  color: isProcessing || forcesExtensionChange ? '#444' : replaceOriginal ? '#f87171' : '#888',
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <label style={{ ...styles.label, marginBottom: 0 }}>Output {activeItem ? `· ${activeItem.file.name.length > 22 ? activeItem.file.name.slice(0, 20) + '…' : activeItem.file.name}` : ''}</label>
                    {activeCustomPath && (
                      <button
                        className="mini-btn"
                        onClick={clearActiveCustomPath}
                        style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '0.72rem', cursor: 'pointer', padding: '1px 4px', borderRadius: '4px' }}
                      >
                        Reset
                      </button>
                    )}
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    <div
                      style={{ fontSize: '0.75rem', color: activeCustomPath ? '#bbb' : '#444', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}
                      title={activeCustomPath || (activeItem ? 'Default: next to original' : 'Select an item first')}
                    >
                      {activeCustomPath ? activeCustomPath.split(/[\\/]/).pop() : activeItem ? 'Next to original' : '—'}
                    </div>
                  </div>
                </div>
              )
            ...
            {isProcessing && (
              <div style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: '#666', marginBottom: '6px' }}>
                  <span style={{ color: '#888' }}>Processing…</span>
                  <span style={{ fontWeight: 600, color: '#10b981' }}>{progress}%</span>
                </div>
                <div style={{ width: '100%', height: '4px', backgroundColor: '#1e1e1e', borderRadius: '4px', overflow: 'hidden' }}>
                  <div
                    className="progress-shimmer"
                    style={{ width: `${progress}%`, height: '100%', borderRadius: '4px', transition: 'width 0.25s ease-out' }}
                  />
                </div>
              </div>
            )}
            ...
            <button
              ...
              style={{
                ...
                background: batchSelection.size > 0 && !isProcessing
                  ? advancedMode
