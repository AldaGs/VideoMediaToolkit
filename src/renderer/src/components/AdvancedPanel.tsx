...
import {
  ...
  type SimpleStateSnapshot,
  buildPreviewArgs,
  formatCommand
} from '../utils/advancedUtils'

export type { AdvancedSettings, SimpleStateSnapshot }
...
interface AdvancedPanelProps {
  ...
  onValidate?: () => Promise<
    { ok: true; command: string } | { ok: false; message: string; command: string }
  >
  presets: Preset[]
...
function categorize(
  ...
  for (const name of commonList) {
    ...
    if (e) common.push(e)
      ...
      common.push({
        name: 'copy',
        description: 'Stream copy (no re-encode)',
        type,
        isHardware: false
      })
  }
...
export function AdvancedPanel({
  ...
  const videoCats = useMemo(
    () =>
      caps
        ? categorize(caps.encoders, 'video', COMMON_VIDEO)
        : { common: [], hardware: [], other: [] },
    [caps]
  )
  const audioCats = useMemo(
    () =>
      caps
        ? categorize(caps.encoders, 'audio', COMMON_AUDIO)
        : { common: [], hardware: [], other: [] },
    [caps]
  )
  ...
  if (error) {
    return (
      <div style={{ padding: '20px 16px', textAlign: 'center' }}>
        <p style={{ color: '#f87171', fontSize: '0.82rem', margin: '0 0 10px' }}>
          Failed to load encoders
        </p>
        <p
          style={{
            color: '#666',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              ? [
                  {
                    label: 'Current',
                    items: [
                      { value: 'custom', label: '— Custom —', description: 'Modified settings' }
                    ]
                  }
                ]
              : []),
            ...
            {presets.map((p) => (
              ...
              >
                ...
                {p.id === activePresetId && (
                  <button
                    onClick={() =>
                      setSaveModal({ open: true, defaultName: p.name, renameId: p.id })
                    }
                    title="Rename preset"
                    style={{
                      background: 'none',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          />
          <span
            style={{ fontSize: '0.72rem', color: dirty ? '#fbbf24' : '#34d399', fontWeight: 500 }}
          >
      ...
      <div>
        ...
        <p style={{ fontSize: '0.68rem', color: '#444', margin: '4px 0 0 0' }}>
          Inserted before output. Don&apos;t include <code style={{ color: '#666' }}>-i</code> or
          output path.
        </p>
      </div>
          ...
          {onValidate && (
            <button
              ...
              style={{
                ...
                color: '#60a5fa',
                fontSize: '0.7rem',
                cursor: validateState.status === 'running' ? 'wait' : 'pointer',
                padding: '3px 10px',
                borderRadius: '5px',
                fontFamily: 'inherit',
                fontWeight: 600,
        ...
        {validateState.status === 'ok' && (
          ...
          >
            ...
            <span style={{ fontSize: '0.75rem', color: '#34d399', fontWeight: 500 }}>
              Validated · 0.5s dry-run succeeded
            </span>
          </div>
        )}
    ...
    >
      ...
      >
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' && !conflict) handleSubmit()
          }}
          ...
          {conflict ? (
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '7px',
                background:
                  trimmed && trimmed !== defaultName
                    ? 'linear-gradient(135deg, #3b82f6, #2563eb)'
                    : '#1a1a1a',
                border: 'none',
                color: trimmed && trimmed !== defaultName ? '#fff' : '#444',
                fontSize: '0.8rem',
                fontWeight: 600,
                cursor: saving
                  ? 'wait'
